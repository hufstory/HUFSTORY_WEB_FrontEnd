import React from "react"

import Header from "../components/Header"
import Footer from "../components/Footer"

//
const Recruit = () => {
  return (
    <div>
      <Header />
      <div className="ml-36 mr-36 mt-8 md:px-5 md:py-5 md:flex-row items-center">
        <div className="flex mt-10 mb-10 text-black body-font bg-transparent relative p-10 rounded-[14px] shadow-default hover:-translate-y-1 hover:scale-100 duration-300">
          <div className="mt-4 block">
            <h1 className="block font-bold text-3xl mb-5">
              2023-1 훕스토리 신입 모집
            </h1>
            <p className="mx-auto mb-6">
              한국외국어대학교 글로벌캠퍼스 학생 IT자치기구 훕스토리 신입회원을
              모집합니다! <br></br>
              신청은 서울, 글로벌 상관 없이 한국외국어대학교 학생이면 누구나
              가능합니다!
            </p>
            <button className="inline-flex mr-auto text-white bg-sky-500 border-0 py-2 px-6 hover:bg-sky-600 rounded text-lg animate-bounce mt-[2px]">
              지원하기
            </button>
          </div>
        </div>
      </div>
      <Footer />
    </div>
  )
}

export default Recruit
